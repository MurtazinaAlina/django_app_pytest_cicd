...
def test_get_course_retrieve(client, courses_factory, url):
    ...
    for index, course in enumerate(courses):
        ...
        # проверка на возврат конкретного курса


@pytest.mark.django_db
def test_get_courses_list(client, courses_factory, url):
    # проверка получения списка курсов (list-логика)
    courses = courses_factory(_quantity=10)
    res = client.get(url)
    assert res.status_code == 200
    # проверка на возврат списка курсов
    assert len(res.json()) == len(courses)

    for index, course in enumerate(res.json()):
        assert course['name'] == courses[index].name
        # проверка на возврат конкретных курсов


@pytest.mark.django_db
def test_get_courses_filter(client, courses_factory, url):
    # проверка фильтрации списка курсов
    courses = courses_factory(_quantity=10)

    for index, course in enumerate(courses):
        res = client.get(url, {'id': course.id})
        assert res.status_code == 200
        # проверка на возврат отфильтрованного по 'id' курса по коду
        assert res.json()[0]['id'] == courses[index].id
        # проверка на возврат конкретного отфильтрованного по 'id' курса'

    for course in courses:
        res = client.get(url, {'name': course.name})
        assert res.status_code == 200
        # проверка на возврат отфильтрованного по 'name' курса по коду
        assert res.json()[0]['name'] == course.name
        # проверка на возврат конкретного отфильтрованного по 'name' курса
